    ...
    const mm = ScrollTrigger.matchMedia({
      ...
      "(min-width: 768px)": function() {
        ...
        timelineCards.fromTo(gsapRefs[0].current, 
          ...
          { x: 800, y: 100, scale: 0.8, opacity: 1, duration: 0.8, ease: "power2.out" }  // 더 빠른 속도
        );

        // 두 번째 카드: y축 이동과 scale 변화 (빠른 속도 설정)
        timelineCards.fromTo(gsapRefs[1].current, 
          { x: -1000, y: 0, scale: 0, opacity: 0 }, 
          { x: 50, y: 25, scale: 0.8, opacity: 1, duration: 0.8, ease: "power2.out" }  // 더 빠른 속도
        );
      },
      ...
      "(max-width: 769px)": function() {
        ...
        timelineCards.fromTo(gsapRefs[0].current, 
          ...
          { x: -30, y: 150, scale: 0.7, opacity: 1, duration: 0.6, ease: "power1.out" }  // 더 빠른 속도
        );

        // 두 번째 카드: 모바일에서의 y축 이동과 크기 변화 (빠른 속도 설정)
        timelineCards.fromTo(gsapRefs[1].current, 
          { x: -500, y: 0, scale: 0, opacity: 0 }, 
          { x: -30, y: 20, scale: 0.7, opacity: 1, duration: 0.6, ease: "power1.out" }  // 더 빠른 속도
        );
      }
    });
